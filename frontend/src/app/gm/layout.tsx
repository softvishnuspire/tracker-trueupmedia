...
import '@/app/gm/dashboard/gm.css';
...
export default function GmLayout({
    ...
    useEffect(() => {
        const checkUser = async () => {
            ...
            try {
                ...
                setClientCount(res.data?.length || 0);
            } catch (err) {
    ...
    // If on /gm/dashboard, GMDashboard manages its own integrated view layout
    if (pathname === '/gm/dashboard') {
        return <>{children}</>;
    }

    const menuItems = [
        { name: 'Dashboard Overview', path: '/gm/dashboard', icon: <LayoutDashboard size={20} /> },
        { name: `Client Management (${clientCount})`, path: '/gm/clients', icon: <Users size={20} /> },
        { name: 'Client Calendar', path: '/gm/dashboard?view=client', icon: <CalendarIcon size={20} /> },
        { name: 'Master Calendar', path: '/gm/dashboard?view=master', icon: <Globe size={20} /> },
        { name: 'Teams', path: '/gm/dashboard?view=teams', icon: <Users size={20} /> },
        { name: 'Employee Management', path: '/gm/dashboard?view=employees', icon: <UserCircle2 size={20} /> },
        { name: 'POC Communication', path: '/gm/dashboard?view=poc', icon: <FileText size={20} /> },
        { name: 'Employee Tracking', path: '/gm/dashboard?view=tracking', icon: <Activity size={20} /> },
        { name: 'Streak System', path: '/gm/dashboard?view=streaks', icon: <Trophy size={20} /> },
    ];
                ...
                <div className="sidebar-footer">
                    ...
                    <div className="user-info-box">
                        ...
                        <div>
                            ...
                            <p className="user-role">{user?.email || 'TrueUp Media'}</p>
                        </div>
                    </div>
