...
import { cooApi, Client } from '@/lib/api';
import { Search, Calendar as CalendarIcon, ChevronRight } from 'lucide-react';

export default function CooClientCalendarsList() {
            ...
            <header className="page-header">
                <div className="header-content">
                    <div>
                        ...
                        <p className="page-subtitle">Monitor individual content schedules for each client</p>
                    </div>
                </div>
            </header>
                ...
                {loading ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(250px, 1fr))', gap: '20px', padding: '20px' }}>
                        {filteredClients.map((client) => (
                            <Link
                                key={client.id}
                                href={`/coo/client-calendar/${client.id}`}
                                className="stat-card"
